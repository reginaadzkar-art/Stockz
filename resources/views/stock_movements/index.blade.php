...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
    <div>
        <h2 class="fw-bold mb-1">Jurnal Mutasi Transaksi Stok</h2>
    ...
    @if(Auth::user()->isAdmin() || Auth::user()->isStaff())
        <div class="d-flex gap-2">
            <a href="{{ route('stock-movements.in.create') }}" class="btn btn-outline-success fw-bold d-flex align-items-center gap-2 px-3 py-2" style="border-color: var(--brand-emerald); color: var(--brand-emerald); border-radius: 10px;">
...
<div class="glass-card mb-4">
    <div class="glass-card-header">
        <h6 class="fw-bold mb-0"><i class="bi bi-arrow-left-right text-emerald me-2"></i>Histori Mutasi Stok ({ $movements->total() } Transaksi)</h6>
        <span class="badge bg-secondary border border-secondary-subtle text-body font-mono">Real-Time Log</span>
    </div>
            ...
            <tbody>
                @forelse($movements as $movement)
                    <tr>
                        ...
                        <td>
                            @if($movement->type === 'in')
                                <span class=""><i class="bi bi-truck me-1 text-muted"></i>{{ $movement->supplier->name ?? '-' }}</span>
                            @else
                                <span class=""><i class="bi bi-person me-1 text-muted"></i>{{ $movement->recipient_or_destination ?? '-' }}</span>
                            @endif
                        </td>
                        <td class="fw-bold font-mono">{{ number_format($movement->total_quantity) }} pcs</td>
                        @if(!Auth::user()->isStaff())
                            <td class="font-mono fw-bold">Rp {{ number_format($movement->total_amount, 0, ',', '.') }}</td>
                        @endif
                        <td>
